...
class PexelsBackground {
  ...
  async loadRandomBackground() {
    const randomQuery =
      this.queries[Math.floor(Math.random() * this.queries.length)];
    ...
    const response = await fetch(
      `https://api.pexels.com/v1/search?query=${randomQuery}&orientation=landscape&size=small&per_page=${randomPerPage}&page=${randomPage}`,
      {
        headers: {
    ...
    if (data.photos && data.photos.length > 0) {
      ...
      const availablePhotos = data.photos.filter(
        (photo) => !this.usedImages.has(photo.id)
      );
  ...
  useFallbackBackground() {
    ...
    const existingBgContainer = heroSection.querySelector(
      ".pexels-bg-container"
    );
